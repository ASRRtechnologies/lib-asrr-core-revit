...
namespace ASRR.Revit.Core.RevitModel
{
    public class ModelElementCopyPaster
    {
        ...
        protected readonly CopyPasteOptions copyOptions = UseDestinationHandler.UseDestinationOnDuplicateNameCopyPasteOptions();
        ...
        protected virtual IEnumerable<ElementId> CopyPasteInSeparateTransactions(Document sourceDoc, Document destinationDoc,
        ...
        {
            _logger.Trace($"Copying modelElements '{string.Join(", ", modelElementIds.Select(id => sourceDoc.GetElement(id).Name))}'");
        ...
        protected void CopyLevels(Document sourceDoc, Document destinationDoc)
        {
            ...
            using (var transaction = WarningDiscardFailuresPreprocessor.GetTransaction(destinationDoc))
            {
                ...
                foreach (var level in sourceLevels.Where(level => destinationLevels.Find(l => l.Name == level.Name) == null))
                {
                    _logger.Trace($"Copying level '{level.Name}' to new file");
                    var idAsList = new List<ElementId> { level.Id };
                    ElementTransformUtils.CopyElements(sourceDoc, idAsList, destinationDoc, Transform.Identity, copyOptions);
                }
